...
step_display = 50
step_save = 10000
path_save = 'resnet18_results_1_2/'
start_from = 'True'
...
def resnet_18(x, train_phase):
    weights = {
        ...
        'wc2a': tf.Variable(tf.random_normal([3, 3, 64, 128], stddev=np.sqrt(2. / (3 * 3 * 64)))), # 2a
        ...
        'wc6a': tf.Variable(tf.random_normal([3, 3, 128 256], stddev=np.sqrt(2. / (3 * 3 * 128)))), # 3a
        ...
        'wc10a': tf.Variable(tf.random_normal([3, 3, 256, 512], stddev=np.sqrt(2. / (3 * 3 * 256)))), # 4a
        ...
        'wc14a': tf.Variable(tf.random_normal([3, 3, 512, 512], stddev=np.sqrt(2. / (3 * 3 * 512)))), # 5a
    ...
    # res2 branch
    res2_branch = tf.nn.conv2d(pool1, weights['wc2a'], strides=[1, 2, 2, 1], padding='SAME')
...
# Construct model
logits = resnet_18(x, train_phase)
...
with tf.Session() as sess:
    ...
    if len(start_from)>1:
        ...
        saver.restore(sess, tf.train.latest_checkpoint("resnet18_results_1/"))
    else:
        sess.run(init)
